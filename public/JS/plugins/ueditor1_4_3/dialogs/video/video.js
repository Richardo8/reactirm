...
(function(){
    ...
    function insertSingle(){
        var width = $G("videoWidth"),
            ...
            url=$G('videoUrl').value.replace(/'/g, '"'),
        ...
        if(/^<iframe/.test(url)){
            ...
            /^[1-9]+[.]?\d*$/g.test( width.value ) ? newIframe.setAttribute("width",width.value) : newIframe.setAttribute("width","94%");
            /^[1-9]+[.]?\d*$/g.test( height.value ) ? newIframe.setAttribute("height",height.value) : newIframe.setAttribute("height","280px");
    ...
    function addUrlChangeListener(url){
        if (browser.ie) {
            url.onpropertychange = function () {
                createPreviewVideo( this.value.replace(/'/g, '"') );
            }
        } else {
    ...
    function createPreviewVideo(url){
        ...
        url = utils.trim(url);
        if(url == ""){
            $G("preview").innerHTML = '<div class="previewMsg"><span>'+lang.urlblankError+'</span></div>'+
